...
export const workOptions = [
  ...
  {
    value: "leatherReconditioning-Interior",
    label: "Leather Reconditioning - Interior",
  },
...
const TimeEntriesComponent: React.FC<TimeEntriesProps> = ({
  ...
  const totalHours = entriesWithIds.reduce(
    ...
    <div className="bg-white shadow-lg rounded-xl p-6 border border-gray-100 w-full relative">
      ...
      {entriesWithIds.length === 0 ? (
        <div className="text-center py-10 text-gray-500 bg-gray-50 rounded-lg">
          ...
          <p>
            No time entries yet. Click `&ldquo;`Add Entry`&ldquo;` to get
            started.
          </p>
        </div>
      ) : (
          ...
          <table className="min-w-full border-collapse">
            <thead>
              <tr className="bg-gray-100">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider border-b">
                  Date
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider border-b">
                  Technician
            ...
            <tbody>
              {entriesWithIds.map((entry) => {
                ...
                const isNewEntry =
                  isEditing && !entry.name && !entry.workPerformed;
                  ...
                  >
                    {isEditing ? (
                      ...
                      <>
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            onChange={(e) =>
                              updateFormField("workPerformed", e.target.value)
                            }
                            className={`w-full p-2 border rounded-md ${
                          ...
                          >
                            ...
                            <optgroup label="Exterior Services">
                              {workOptions
                                .filter((opt) => opt.value.includes("Exterior"))
                                .map((option) => (
                                  <option
                                    key={option.value}
                                    value={option.value}
                            ...
                            <optgroup label="Interior Services">
                              {workOptions
                                .filter((opt) => opt.value.includes("Interior"))
                                .map((option) => (
                                  <option
                                    key={option.value}
                                    value={option.value}
                      ...
                      <>
                        ...
                        <td className="px-4 py-3">
                          {workOptions.find(
                            (opt) => opt.value === entry.workPerformed
                          )?.label || entry.workPerformed}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex space-x-2">
                            <button
      ...
      {entriesWithIds.length > 0 && (
        <div className="mt-6 pt-4 flex justify-between items-center border-t border-gray-200">
          <div className="text-sm text-gray-500">
            {entriesWithIds.length}{" "}
            {entriesWithIds.length === 1 ? "entry" : "entries"}
          </div>
